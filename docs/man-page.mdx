...
DESCRIPTION         top
   ...
   Extended BPF Design/Architecture
       ...
       programs attached to a tc(8) classifier), and other types that may
   ...
   Arguments
       ...
       union of type bpf_attr (see below).  The unused fields and padding
       must be zeroed out before the call.  The size argument is the size
       of the union pointed to by attr.
   ...
   eBPF maps
       ...
       of data.  They allow sharing of data between eBPF kernel programs,
       ...
       BPF_MAP_UPDATE_ELEM
              ...
              map reached the max_entries limit specified at map creation
              time.  EEXIST will be returned if flags specifies
       ...
       BPF_MAP_GET_NEXT_KEY
              The BPF_MAP_GET_NEXT_KEY command looks up an element by key
              in the map referred to by the file descriptor fd and sets
              ...
              pointer to the key of the first element.  If key is the
              last element, -1 is returned and errno is set to ENOENT.
              Other possible errno values are ENOMEM, EFAULT, EPERM, and
   ...
   eBPF map types
       ...
       BPF_MAP_TYPE_PROG_ARRAY (since Linux 4.2)
              ...
              eBPF programs.  Thus, both the key_size and value_size must
   ...
   eBPF programs
       ...
       kernel.  The return value for this command is a new file
       ...
       •  log_size size of the buffer pointed to by log_buf.  If the size
   ...
   Events
       ...
       prog_fd to the socket sockfd, which was created by an earlier call
       to socket(2):
...
NOTES         top
       ...
       duplicated in the usual way, using dup(2) and similar calls.  An
       ...
       the samples/bpf/*_kern.c files in the kernel source tree.
